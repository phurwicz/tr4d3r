...
class Tentative(Loggable):
    def __init__(
        self, condition, trigger, order_time=None, expire_after=86400, verbose=False
    ):
        """
        ...
        assert not condition(
            order_time
        ), "Condition is not expected to be met immediately"
    ...
    def __call__(self, time):
        ...
        assert (
            time >= last_time
        ), f"Illegal limit order state: old time {last_time}, new time {time}"
