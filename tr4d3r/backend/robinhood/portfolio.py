...
class RobinhoodRealPortfolio(RealTimePortfolio):
    ...
    def resolve_filled_order(self, order):
        ...
        assert (
            len(order["executions"]) == 1
        ), f"Expected exactly one execution, got {order['executions']}"
    ...
    def refresh_single_open_order(self, order_id):
        ...
        assert (
            order["type"] == "market"
        ), f"Expected a market order, got {order['type']}"
        ...
        if state in {"unconfirmed", "confirmed", "queued"}:
            ...
            self._fail(f"Order canceled: {brief}")
            self.open_orders.remove(order_id)
        else:
    ...
    def validate_cash(self):
        ...
        assert (
            cash.name == self.__class__.CASH_NAME
        ), f"Unexpected cash name: {cash.name}"
        assert (
            cash.quantity <= available_cash
        ), f"Expected cash >= {cash.quantity}, got {available_cash}"
    ...
    def validate_items(self):
        ...
        for _item in self.item_dict.values():
            assert (
                _item.name in holdings_dict
            ), f"Expected item {_item.name} in holdings"
            _online_dict = holdings_dict[_item.name]

            _offline_q = _item.quantity
            _online_q = float(_online_dict["quantity"])
            assert (
                _offline_q == _online_q
            ), f"Expected {_item.name} quantity {_offline_q}, got {_online_q}"

            _offline_c = _item.unit_cost
            _online_c = float(_online_dict["average_buy_price"])
            assert (
                _offline_q == _online_q
            ), f"Expected {_item.name} unit cost {_offline_c}, got {_online_c}"
    ...
    def local_deposit(self, amount):
        ...
        ), f"Expected cash >= {target}, got {available_cash}"
    ...
    def local_withdraw(self, amount):
        ...
        ), f"Not enough cash ({self.cash.quantity}) to withdraw ({amount})"
        self.principal.quantity -= amount
        self.cash.quantity -= amount
