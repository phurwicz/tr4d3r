...
class PseudoTimeMarket(Loggable):
    ...
    def time_sequence(self, offset):
        ...
        assert (
            sorted(offset_list) == offset_list
        ), "Offset must be sorted in ascending order"
        first = offset_list[0]
        assert isinstance(
            first, timedelta
        ), f"Expected timedelta, got {type(first)} {first}"
        return offset_list
    ...
    def get_shares(self, price, shares=None, amount=None):
        assert isinstance(price, int) or isinstance(
            price, float
        ), f"Expected price as int or float, got {type(price)} {price}"
        if shares is not None:
            assert amount is None, "Expected exactly one of 'shares' and 'amount'"
            assert isinstance(shares, int) or isinstance(
                shares, float
            ), f"Expected shares as int or float, got {type(shares)} {shares}"
        else:
            assert isinstance(
                amount, float
            ), f"Expected amount to be float, got {type(amount)} {amount}"
    ...
    def market_bid(self, symbol, shares=None, amount=None, time=None):
        ...
        item = PseudoTimeItem(
            name=symbol, quantity=shares, unit_cost=price, market=self
        )
