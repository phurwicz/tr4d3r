...
class Portfolio(Loggable):
    ...
    def to_dict(self):
        data_dict = {
            ...
            "items": {
                _item.name: _item.to_dict()
                for _item in sorted(self.item_dict.values(), key=lambda x: x.name)
            },
        }
        return data_dict
    ...
    def from_dict(cls, data_dict):
        ...
        ), f"Class name mismatch: {class_name} vs. {cls.__name__}"

        # constructor
        principal = cls.DEFAULT_CASH_CLASS.from_dict(prin_dict)
        cash = cls.DEFAULT_CASH_CLASS.from_dict(cash_dict)
        items = [
            cls.DEFAULT_ITEM_CLASS.from_dict(_item_d) for _item_d in items_dict.values()
        ]
    ...
    def cost_detail(self):
        detail = {
            ...
            "items": [
                {"name": _item.name, "cost": _item.cost()}
                for _item in self.item_dict.values()
            ],
        }
        return detail
...
class PseudoTimePortfolio(Portfolio):
    ...
    def worth_detail(self, time):
        detail = {
            ...
            "items": [
                {"name": _item.name, "worth": _item.worth(time)}
                for _item in self.item_dict.values()
            ],
        }
        return detail
    ...
    def withdraw(self, amount, time):
        ...
        self.cash.quantity -= amount
        self._info(
            f"Cash withdrawal {amount} @ {time} | balance = {self.cash.quantity}"
        )

    def tick(self, time):
        gap_seconds = (
            self.__class__.DEFAULT_GAP_SECONDS
            if self.last_tick_time is None
            else (time - self.last_tick_time).total_seconds()
        )
        assert (
            gap_seconds > 0
        ), f"Unexpected timeline: last move @ {self.last_tick_time}, tick @ {time}"
        self.last_tick_time = time
        return gap_seconds

    def market_buy(self, symbol, shares=None, amount=None, time=None):
        time = autostamp(time)
        item, cash = self.market.market_ask(
            symbol, shares=shares, amount=amount, time=time
        )
        if self.verbose:
            self._good(
                f"{datetime.fromtimestamp(time)}: B {round(item.quantity, 6)} {item.name} <- {round(cash.quantity, 6)} {cash.name}."
            )
        self.cash.sub(cash)
        self.add_item(item)
    ...
    def market_sell(self, symbol, shares=None, amount=None, time=None):
        time = autostamp(time)
        item, cash = self.market.market_bid(
            symbol, shares=shares, amount=amount, time=time
        )
        if self.verbose:
            self._good(
                f"{datetime.fromtimestamp(time)}: S {round(item.quantity, 6)} {item.name} -> {round(cash.quantity, 6)} {cash.name}."
            )
        self.sub_item(item)
        self.cash.add(cash)
    ...
    def limit_buy(
        self, symbol, shares, limit_price, order_time=None, expire_after=86400
    ):
        ...
        condition = NamedCallback(condition_f, f"{symbol} P<={round(limit_price, 6)}")
        trigger = NamedCallback(trigger_f, f"B {round(shares, 6)} * {symbol}")
        callback = Tentative(
            condition=condition,
            trigger=trigger,
    ...
    def limit_sell(
        ...
        condition = NamedCallback(condition_f, f"{symbol} P>={round(limit_price, 6)}")
        trigger = NamedCallback(trigger_f, f"S {round(shares, 6)} * {symbol}")
        callback = Tentative(
            condition=condition,
            trigger=trigger,
            order_time=autostamp(order_time),
...
class RealTimePortfolio(Portfolio):
    ...
    def open_orders(self, orders):
        raise ValueError(
            "It is forbidden to directly set open orders. Please use set methods such as add() or remove()."
        )
    ...
    def worth_detail(self):
        detail = {
            ...
            "cash": self.cash.__repr__(),
            "items": [
                {"name": _item.name, "worth": _item.worth()}
                for _item in self.item_dict.values()
            ],
        }
        return detail
    ...
    def tick(self, update):
        ...
        gap_seconds = (
            self.__class__.DEFAULT_GAP_SECONDS
            if self.last_tick_time is None
            else (now_utc - self.last_tick_time).total_seconds()
        )
        assert (
            gap_seconds > 0
        ), f"Unexpected timeline: last move @ {self.last_tick_time}, tick @ {now_utc}"
        assert isinstance(
            update, bool
        ), f"Expected boolean value, got {type(update)} {update}"
        if update:
    ...
    def postprocess_new_order(self, order):
        if order is not None:
            if isinstance(order, dict):
                ...
                self._fail(f"Expected an order dict, got {order}")
        return order
