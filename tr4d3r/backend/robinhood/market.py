...
@wrappy.memoize(
    cache_limit=100,
    return_copy=False,
    persist_path="robin-history.pkl",
...
def rh_historicals(symbol, interval, span):
    ...
    assert (
        isinstance(dictl, list) and dictl
    ), f"Expected non-empty list of dicts, got {dictl}"
...
class RobinhoodPseudoMarket(PseudoTimeMarket):
    ...
    def __init__(self, symbols, interval="day", span="5year"):
        ...
        for _symbol in tqdm(symbols, desc="Loading stock historicals"):
            ...
            _table["begins_at_timestamp"] = _table["begins_at"].apply(
    ...
    def get_price(self, symbol, time=None):
        ...
        interval_idx = (
            np.searchsorted(
                table["begins_at_timestamp"].values, time.timestamp() + 1e-6
            )
            - 1
        )
...
class RobinhoodRealMarket(RealTimeMarket):
    ...
    def validate_order(self, symbol, shares, amount):
        ...
        if (
            ...
            or amount < self.__class__.MIN_TRANS_AMOUNT
        ):
    ...
    def get_bid_price(self, symbol):
        price_list = rh.stocks.get_latest_price(
            symbol, "bid_price", includeExtendedHours=False
        )
        return list(map(float, price_list))[0]

    def get_ask_price(self, symbol, time=None):
        price_list = rh.stocks.get_latest_price(
            symbol, "ask_price", includeExtendedHours=False
        )
        return list(map(float, price_list))[0]
